...
class HistoryBCNodeDB
{
	...
	{
		this.accessor.getPrimaryKey().put(new HistoryBCNodeEntity(DBConfig.getHistoryBCKey(node.getKey(), version), node.getKey(), version, node.getName(), node.getIP().getIP(), node.getIP().getPort(), node.getServiceName(), node.getPartners(), node.getDescription()));
	}

	public BCNode getNode(String nodeKey, int version)
	{
		HistoryBCNodeEntity entity = this.accessor.getPrimaryKey().get(DBConfig.getHistoryBCKey(nodeKey, version));
		if (entity != null)
		{
	...
	public BCNode getNode(String key)
	{
	...
	public int getVersionSize(String nodeKey)
	{
		...
		SecondaryIndex<String, String, HistoryBCNodeEntity> sIndex = this.env.getEntityStore().getSecondaryIndex(primaryIndex, String.class, DBConfig.NODE_KEY);
		EntityJoin<String, HistoryBCNodeEntity> join = new EntityJoin<String, HistoryBCNodeEntity>(primaryIndex);
		join.addCondition(sIndex, nodeKey);
